...
export function mergeSubtitles(chineseInput: string, englishInput: string, options: Partial<MergeOptions> = {}): MergeResult {
  ...
  const ass = renderAss(normalizeTimeline(output, opts), opts);
  return {
    ass,
    stats: {
...
function normalizeTimeline(cues: Array<{ start: number; end: number; text: string }>, opts: MergeOptions) {
  if (!opts.preventOverlap) return cues;
  const sorted = [...cues].sort((a, b) => a.start - b.start || a.end - b.end);
  return sorted.map((cue, index) => {
    const next = sorted[index + 1];
    if (!next || cue.end <= next.start) return cue;
    const trimmedEnd = Math.max(cue.start + 0.08, next.start - 0.02);
    return { ...cue, end: Math.min(cue.end, trimmedEnd) };
